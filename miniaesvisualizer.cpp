...
const int MiniAESVisualizer::INVSBOX[256] = {
...
static void invSubBytes(int state[2][2]) {
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 2; j++)
            state[i][j] = MiniAESVisualizer::INVSBOX[state[i][j] & 0xFF];
}
...
static void invMixColumns(int state[2][2]) {
    int t0 = state[0][0];
    int t1 = state[0][1];
    state[0][0] = state[1][0];
    state[0][1] = state[1][1];
    state[1][0] = t0 ^ state[1][0];
    state[1][1] = t1 ^ state[1][1];
}
...
void MiniAESVisualizer::decrypt(int cipher[2][2], int key[2][2], int result[2][2]) {
    int state[2][2];
    copyState(state, cipher);
...
QList<AesStep> MiniAESVisualizer::generateDecryptSteps(int cipher[2][2], int key[2][2]) {
    QList<AesStep> steps;
    int state[2][2];
    copyState(state, cipher);
    int before[2][2];
    ...
    // Round 2 inverse: AddRoundKey (with RK2)
    getRoundKey(key, rk, 2);
    copyState(before, state);
    step.name = "第 2 轮逆：轮密钥加 (AddRoundKey)";
    step.detail = QString("状态矩阵 ⊕ RoundKey2\n")
        + QString("%1⊕%2=%3  %4⊕%5=%6\n")
              .arg(toHex(before[0][0])).arg(toHex(rk[0][0])).arg(toHex(before[0][0]^rk[0][0]))
              .arg(toHex(before[0][1])).arg(toHex(rk[0][1])).arg(toHex(before[0][1]^rk[0][1]))
        + QString("%1⊕%2=%3  %4⊕%5=%6")
    ...
    // Round 2 inverse: InvShiftRows
    copyState(before, state);
    step.name = "第 2 轮逆：逆行移位 (InvShiftRows)";
    step.detail = QString("第 1 行循环右移一位（恢复原位）\n")
        + QString("[%1 %2] → [%3 %4]")
              .arg(toHex(before[1][0])).arg(toHex(before[1][1]))
              .arg(toHex(before[1][1])).arg(toHex(before[1][0]));
    copyState(step.before, before);
    ...
    // Round 2 inverse: InvSubBytes
    copyState(before, state);
    step.name = "第 2 轮逆：逆字节替换 (InvSubBytes)";
    step.detail = QString("逆 S-Box 字节替换（恢复原始字节值）\n")
        + QString("SBox⁻¹[%1]=%2  SBox⁻¹[%3]=%4\n")
              .arg(toHex(before[0][0])).arg(toHex(INVSBOX[before[0][0] & 0xFF]))
              .arg(toHex(before[0][1])).arg(toHex(INVSBOX[before[0][1] & 0xFF]))
        + QString("SBox⁻¹[%1]=%2  SBox⁻¹[%3]=%4")
              .arg(toHex(before[1][0])).arg(toHex(INVSBOX[before[1][0] & 0xFF]))
              .arg(toHex(before[1][1])).arg(toHex(INVSBOX[before[1][1] & 0xFF]));
    copyState(step.before, before);
    invSubBytes(state);
    copyState(step.after, state);
    steps.append(step);

    // Round 1 inverse: AddRoundKey (with RK1)
    getRoundKey(key, rk, 1);
    copyState(before, state);
    step.name = "第 1 轮逆：轮密钥加 (AddRoundKey)";
    step.detail = QString("状态矩阵 ⊕ RoundKey1\n"
        "RoundKey1[0][0] = 原始密钥[0][0] ⊕ 0x01\n")
        + QString("%1⊕%2=%3  %4⊕%5=%6\n")
              .arg(toHex(before[0][0])).arg(toHex(rk[0][0])).arg(toHex(before[0][0]^rk[0][0]))
    ...
    // Round 1 inverse: InvMixColumns
    copyState(before, state);
    step.name = "第 1 轮逆：逆列混合 (InvMixColumns)";
    step.detail = QString("逆列混合（应用 MixColumns 的逆运算）\n")
        + QString("运算规则：(x, y) → (y, x⊕y)\n")
        + QString("[%1 %2]  [%3 %4]\n")
              .arg(toHex(before[0][0])).arg(toHex(before[0][1]))
              .arg(toHex(before[1][0])).arg(toHex(before[1][1]))
        + QString("→ [%3 %4]  [%1⊕%3 %2⊕%4]")
              .arg(toHex(before[0][0])).arg(toHex(before[0][1]))
              .arg(toHex(before[1][0])).arg(toHex(before[1][1]));
    ...
    // Round 1 inverse: InvShiftRows
    copyState(before, state);
    step.name = "第 1 轮逆：逆行移位 (InvShiftRows)";
    step.detail = QString("第 1 行循环右移一位（恢复原位）\n")
        + QString("[%1 %2] → [%3 %4]")
              .arg(toHex(before[1][0])).arg(toHex(before[1][1]))
              .arg(toHex(before[1][1])).arg(toHex(before[1][0]));
    ...
    // Round 1 inverse: InvSubBytes
    copyState(before, state);
    step.name = "第 1 轮逆：逆字节替换 (InvSubBytes)";
    step.detail = QString("逆 S-Box 字节替换（恢复原始字节值）\n")
        + QString("SBox⁻¹[%1]=%2  SBox⁻¹[%3]=%4\n")
              .arg(toHex(before[0][0])).arg(toHex(INVSBOX[before[0][0] & 0xFF]))
              .arg(toHex(before[0][1])).arg(toHex(INVSBOX[before[0][1] & 0xFF]))
    ...
    // Round 0 inverse: AddRoundKey (with RK0)
    getRoundKey(key, rk, 0);
    copyState(before, state);
    step.name = "第 0 轮逆：轮密钥加 —— 最终明文！";
    step.detail = QString("状态矩阵 ⊕ RoundKey0（原始密钥）—— 还原为明文！\n")
        + QString("%1⊕%2=%3  %4⊕%5=%6\n")
              .arg(toHex(before[0][0])).arg(toHex(rk[0][0])).arg(toHex(before[0][0]^rk[0][0]))
              .arg(toHex(before[0][1])).arg(toHex(rk[0][1])).arg(toHex(before[0][1]^rk[0][1]))
        + QString("%1⊕%2=%3  %4⊕%5=%6")
...
MiniAESVisualizer::MiniAESVisualizer(int plain[2][2], int key[2][2], Mode mode, QWidget *parent)
    ...
    if (mode == Decrypt) {
        m_steps = generateDecryptSteps(plain, key);
    } else {
        m_steps = generateSteps(plain, key);
    }

    setWindowTitle(mode == Decrypt ? "Mini-AES 解密推演 (Inverse S-Box)"
                                   : "Mini-AES 加密推演 (AES S-Box)");
...
void MiniAESVisualizer::setupUI() {
    ...
    // Title
    QLabel *title = new QLabel(m_mode == Decrypt ? "Mini-AES 解密推演" : "Mini-AES 加密推演", this);
